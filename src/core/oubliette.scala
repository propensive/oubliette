...
class Jvm(val funnel: Funnel[Text], val task: Task[Unit], val process: Process[Text]):
  ...
  def kill(): Unit = funnel.put(t"exit\t2\n")
...
object Jvm:
  ...
  def fork()(using log: Log, monitor: Monitor): Jvm throws IoError | StreamCutError | EnvError =
